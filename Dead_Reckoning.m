function [ins_att_plus,ins_vel_plus,ins_pos_plus,ins_pos_plus_xyz] = Dead_Reckoning(ins_att_minus,ins_vel_minus,ins_pos_minus,omega_b_ib,v_ib_b,dt)
...
C_n_b_minus = angle2dcm(ins_att_minus(1),ins_att_minus(2),ins_att_minus(3),'XYZ');
...
[ins_att_plus(1,1),ins_att_plus(2,1),ins_att_plus(3,1)] = dcm2angle(C_n_b_plus,'XYZ');
...
ins_vel_plus = ins_vel_minus+V_n_ib+([0;0;gravity(norm(llh2xyz(ins_pos_minus)))]-(Omega_n_en+2*Omega_n_ie)*ins_vel_minus)*dt;
...
% Convert from curvilinear to cartesian coordinates
ins_pos_plus_xyz = llh2xyz(ins_pos_plus);
