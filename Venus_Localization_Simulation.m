...
% Venus_Localization_Simulation.m
% 
...
disp('Simulating...')
parfor n = 1:count
    [trials{n}] = VLS(n,e_wind,n_wind,u_wind,topography,barometry);
end
...
function [trial] = VLS(n,e_wind,n_wind,u_wind,topography,barometry)

    % Parameters

    % Time
    time = 60*60; % Total time of simulation in seconds
    ...
    % Process Noise
    sigma_roll = deg2rad(0.1); % Process noise of roll angle [deg]
    sigma_pitch = deg2rad(0.1); % Process noise of pitch angle [deg]
    sigma_yaw = deg2rad(0.1); % Process noise of yaw angle [deg]
    sigma_u = 0.1; % Process noise of velocity in north direction [m/s]
    sigma_v = 0.1; % Process noise of velocity in east direction [m/s]
    sigma_w = 0.1; % Process noise of velocity in down direction [m/s]
    sigma_lat = 1*(1/6051800); % Process noise of position in latitude direction [m]
    sigma_lon = 1*(1/6051800); % Process noise of position in longitude direction [m]
    sigma_h = 1; % Process noise of position in height direction [m]
    sigma_gyro_bias = 0.1*(pi/180)*(1/3600); % Process noise of gyroscope bias [rad/s]
    sigma_accel_bias = 0.1*(8.858/1000); % Process noise of accelerometer bias [m/s^2]
